...
- During session wrapup, record editorial provenance in `[extra.editorial]` and `[[extra.editorial.provenance]]`: identify the affected field, the source categories used, whether AI assisted, and whether a human reviewed it. Keep page rendering unchanged.
...
- After wrapping a session with Lichess games, run `just update-session-capivaradas NNNN` so the page gets the Lichess blunder replay data before the final build. This is expected to be a normal quick wrap step; if Lichess is unavailable, report it and continue with the rest of the wrap.
